...
namespace Archive
{
    ...
    public static class ToArchive
    {
        static Dictionary<string, string> AddFolderSubfiles(Dictionary<string, string> folders)
        {
            ...
            AddFilesfromCurrent = path =>
                 {
                     if(Directory.Exists(path))
                     {
                         foreach(var file in Directory.GetFiles(path).Where(file => !folders.ContainsKey(file)))
                             folders.Add(file, String.IsNullOrEmpty(folders[path]) ? Path.GetFileName(file) : folders[path]+"/"+ Path.GetFileName(file));
                         foreach(var dir in Directory.GetDirectories(path).Where(dir => !folders.ContainsKey(dir)))
                         {
                             folders.Add(dir, folders[path] +"/"+ Path.GetFileName(dir));
                             AddFilesfromCurrent(dir);
                         }
                         folders.Remove(path);
                     }
                     else
                         
                         folders[path] = String.IsNullOrEmpty(folders[path]) ? Path.GetFileName(path) : folders[path] + "/" + Path.GetFileName(path);
